<x-app-layout>
    <x-slot name="header">
        <h2 class="text-xl font-semibold leading-tight text-gray-800">edit Class Room</h2>
    </x-slot>
                        ...
                        <form method="POST" action="{{ route('classRooms.update', $classroom->id) }}">
                            ...
                            <div class="grid w-full grid-cols-2 gap-4">
                                <div class="w-full">
                                    ...
                                    <x-text-input value="{{old('name',$classroom->name)}}" name='name' class="w-full"></x-text-input>
                                    @error('name')
                                ...
                                <div class="w-full">
                                    ...
                                    <x-text-input value="{{old('configration',$classroom->configration)}}" name='configration' class="w-full"></x-text-input>
                                    @error('configration')
                                        <div class="font-bold text-red-600">{{ $message }}</div>
                                    @enderror
                                </div>
                                <div class="w-full">
                                    ...
                                    <x-text-input value="{{old('capacity',$classroom->capacity)}}" name='capacity' class="w-full"></x-text-input>
                                    @error('capacity')
                                        <div class="font-bold text-red-600">{{ $message }}</div>
                                    @enderror
                                </div>
                                <div class="w-full">
                                    ...
                                    <select name="branch_id">
                                        ...
                                        <option value="{{ $id }}" {{ $id == old('branch_id',$classroom->branch_id)? 'selected':''}}>{{ $name}}</option>
                                        @endforeach
                                    </select>
                                    @error('branch_id ')
                                        <div class="font-bold text-red-600">{{ $message }}</div>
