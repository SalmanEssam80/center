                        ...
                        <form method="POST" action="{{ route('category.update',$category->id) }}">
                            ...
                            <div class="grid w-full grid-cols-2 gap-4">
                                ...
                                <div class="w-full">
                                    ...
                                    <x-text-input value="{{old('image',$category->image)}}" name='image' class="w-full"></x-text-input>
                                    @error('image')
                                        <div class="font-bold text-red-600">{{ $message }}</div>
                                    @enderror
                                </div>
                                <div class="w-full">
                                    ...
                                    <select name="category_id">
                                        <option selected value="{{ old('category_id',$category->category_id) }}">{{ $category->category_id}}</option>
                                        @foreach (App\models\Category::orderBy('name')->pluck('name', 'id')->toArray() as $id => $name)
                                        <option value="{{ old('company_id',$category->category_id) }}">{{ $name}}</option>
                                        @endforeach
                                    </select>
                                    @error('category_id')
                                        <div class="font-bold text-red-600">{{ $message }}</div>
